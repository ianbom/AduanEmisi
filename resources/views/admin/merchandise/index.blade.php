    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Merchandise</h1>
                    <p class="text-gray-600 text-lg">Kelola dan pantau semua merchandise reward yang tersedia</p>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $merchandise->where('is_active', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $merchandise->where('is_active', false)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($merchandise->avg('points_cost'), 0) }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form method="GET" action="{{ route('admin.merchandise.index') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="is_active" id="is_active" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                        ...
                        <!-- Search Filter -->
                        <div class="space-y-2">
                            <label for="search" class="block text-sm font-semibold text-gray-700">Cari Merchandise</label>
                            <div class="relative">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex space-x-3">
                                <input type="number" name="points_min" value="{{ request('points_min') }}" placeholder="Min"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                <input type="number" name="points_max" value="{{ request('points_max') }}" placeholder="Max"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            </div>
                        </div>
                    </div>
        ...
        <div class="mb-6 flex justify-end">
            <div class="bg-white rounded-lg p-1 shadow-sm border border-gray-200">
                <button id="gridView" class="px-4 py-2 text-sm font-medium text-gray-700 rounded-md bg-blue-100 text-blue-700">
                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path>
                    </svg>
                    Grid
                </button>
                <button id="tableView" class="px-4 py-2 text-sm font-medium text-gray-700 rounded-md hover:bg-gray-100">
                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="merchandiseGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300">
                    <div class="relative">
                        ...
                        <div class="absolute top-3 right-3">
                            @if($item->is_active)
                                <span class="bg-green-100 text-green-800 text-xs font-semibold px-2 py-1 rounded-full border border-green-200">
                                    Aktif
                                </span>
                            @else
                                <span class="bg-red-100 text-red-800 text-xs font-semibold px-2 py-1 rounded-full border border-red-200">
                                    Tidak Aktif
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-gray-600 text-sm mb-4 line-clamp-2" title="{{ $item->description }}">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center">
                                ...
                                <span class="text-lg font-bold text-gray-900">{{ number_format($item->points_cost) }}</span>
        ...
        <div id="merchandiseTable" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hidden">
                        ...
                        @foreach($merchandise as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                    <span class="bg-gray-100 px-2 py-1 rounded-lg">#{{ $item->id }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <div class="max-w-xs">
                                        <p class="truncate" title="{{ $item->description }}">{{ Str::limit($item->description, 50) }}</p>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                    <div class="flex items-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full border bg-red-100 text-red-800 border-red-200">
                                            Tidak Aktif
                                        </span>
                                    @endif
                                </td>
    ...
    $(document).ready(function() {
        ...
        let merchandiseTable = $('#merchandiseDataTable').DataTable({
            ...
            columnDefs: [
                ...
                { orderable: false, targets: 1 }   // Disable ordering on image column
            ],
            pageLength: 10,
            language: {
                search: "Cari:",
        ...
        if (window.innerWidth < 768) {
            const hasActiveFilters = {{ request()->hasAny(['is_active', 'search', 'points_min', 'points_max']) ? 'true' : 'false' }};
