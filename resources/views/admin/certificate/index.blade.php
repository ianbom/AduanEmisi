    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Sertifikat</h1>
                    <p class="text-gray-600 text-lg">Kelola semua template sertifikat yang tersedia.</p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 col-span-1 md:col-span-4">
            ...
            <div id="filterForm" class="p-6">
                <form method="GET" action="{{ route('admin.certificates.index') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Search Filter -->
                        <div class="space-y-2">
                            <label for="search" class="block text-sm font-semibold text-gray-700">Cari Sertifikat</label>
                            <div class="relative">
                        ...
                        @foreach($certificates as $certificate)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                    <span class="bg-gray-100 px-2 py-1 rounded-lg">#{{ $certificate->id }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <div class="max-w-xs">
                                        <p class="truncate font-medium" title="{{ $certificate->name }}">{{ $certificate->name }}</p>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <div class="max-w-xs">
                                        <p class="truncate" title="{{ $certificate->template }}">{{ $certificate->template }}</p>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        <a href="{{ route('admin.certificates.show', $certificate->id) }}"
                                           class="text-blue-600 hover:text-blue-800 transition-colors duration-150" title="Lihat">
                                        ...
                                        <a href="{{ route('admin.certificates.edit', $certificate->id) }}"
                                           class="text-green-600 hover:text-green-800 transition-colors duration-150" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    $(document).ready(function() {
        ...
        $('#certificateTable').DataTable({
            order: [[0, 'desc']], // Urutkan berdasarkan kolom ID (kolom ke-0) secara descending
            columnDefs: [
                { orderable: false, targets: 4 } // Nonaktifkan pengurutan pada kolom Aksi (kolom ke-4)
            ],
