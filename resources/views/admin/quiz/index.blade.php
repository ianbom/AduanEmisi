    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Quiz</h1>
                    <p class="text-gray-600 text-lg">Kelola dan pantau semua quiz yang tersedia</p>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $quizzes->where('is_active', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $quizzes->where('difficulty', 'sulit')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div id="filterForm" class="p-6" style="display: none;">
                <form method="GET" action="{{ route('admin.quizzes.index') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <!-- Difficulty Filter -->
                        <div class="space-y-2">
                            <label for="difficulty" class="block text-sm font-semibold text-gray-700">Tingkat Kesulitan</label>
                            <select name="difficulty" id="difficulty" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-6 border-t border-gray-100">
                        <a href="{{ route('admin.quizzes.index') }}" class="px-6 py-3 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 font-medium text-center">Reset Filter</a>
                        ...
                        <a href="{{ route('admin.quizzes.create') }}" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium inline-flex items-center">
                ...
                <table id="quizTable" class="min-w-full" style="width:100%">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Thumbnail</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Judul</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kesulitan</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($quizzes as $quiz)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold">#{{ $quiz->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($quiz->thumbnail_url)
                                        <img src="{{ Storage::url($quiz->thumbnail_url) }}" alt="Thumbnail" class="h-12 w-12 rounded-lg object-cover">
                                    @else
                                        <div class="h-12 w-12 bg-gray-200 rounded-lg flex items-center justify-center"><svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm"><p class="font-medium truncate" title="{{ $quiz->title }}">{{ $quiz->title }}</p></td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $difficultyColors = ['mudah' => 'bg-green-100 text-green-800', 'sedang' => 'bg-yellow-100 text-yellow-800', 'sulit' => 'bg-red-100 text-red-800'];
                                    @endphp
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $difficultyColors[$quiz->difficulty] ?? 'bg-gray-100' }}">{{ ucfirst($quiz->difficulty) }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold">{{ $quiz->points_reward }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($quiz->is_active)
                                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Aktif</span>
                                    @else
                                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Non-Aktif</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $quiz->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        <a href="{{ route('admin.quizzes.show', $quiz->id) }}" class="text-blue-600 hover:text-blue-800" title="Lihat Detail"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></a>
                                        <a href="{{ route('admin.quizzes.edit', $quiz->id) }}" class="text-green-600 hover:text-green-800" title="Edit Quiz"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                        <form action="{{ route('admin.quizzes.destroy', $quiz->id) }}" method="POST" class="inline">@csrf @method('DELETE') <button type="submit" class="text-red-600 hover:text-red-800" title="Hapus Quiz" onclick="return confirm('Hapus quiz ini?')"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></form>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
...
<script>
    $(document).ready(function() {
        ...
        $('#quizTable').DataTable({
            order: [[0, 'desc']],
            columnDefs: [
                { orderable: false, targets: [1, -1] } // Disable ordering on thumbnail and actions columns
            ],
             language: {
        ...
        if (window.innerWidth < 768) {
            const hasActiveFilters = {{ request()->hasAny(['difficulty', 'search', 'status', 'date_from', 'date_to', 'points_min', 'points_max']) ? 'true' : 'false' }};
            if (!hasActiveFilters) {
                $('#filterForm').hide();
