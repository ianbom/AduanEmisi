...
export function ForgotPasswordForm({
  ...
  // Mengambil flash message 'status' dari props halaman
  const { props: pageProps } = usePage<PageProps>();

  // --- PERBAIKAN DI SINI ---
  // Gunakan optional chaining (?.) untuk mengakses properti flash dengan aman.
  // Jika pageProps.flash adalah undefined, maka status akan menjadi undefined tanpa error.
  const status = pageProps.flash?.status;

  // Inisialisasi form hanya dengan email, sesuai kebutuhan controller
          ...
          <form className="p-6 md:p-8" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-6">
              <div className="flex flex-col items-center text-center">
                <h1 className="text-2xl font-bold text-green-900">Lupa Password?</h1>
              ...
              <div className="flex items-center gap-4">
                <Button asChild variant="link" className="p-0 text-sm h-auto">
