...
const Button = ({ children, variant = 'default', size = 'md', className = '', ...props }: any) => {
    // ... implementasi Button Anda yang sudah ada ...
...
const CommentUploadCard = ({ reportId }: CommentUploadCardProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // 2. Kirim data ke backend menggunakan fungsi post dari useForm
        post(route('comments.store'), {
            forceFormData: true, // Wajib untuk file upload
            onSuccess: () => {
                reset(); // Reset semua field form setelah berhasil
            },
                        ...
                        <div className="mb-4 mt-2">
                            {preview ? (
                                ...
                                <div className="relative inline-block">
                                    <img src={preview} alt="Preview" className="object-contain h-auto max-w-full border border-gray-200 rounded-lg max-h-48" />
                                    <button type="button" onClick={removeFile} className="absolute p-1 text-white bg-red-500 rounded-full -right-2 -top-2 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500">
                                        <X size={14} />
                                    </button>
                                </div>
                            ) : (
                                ...
                                <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50">
                                    <div className="flex items-center space-x-3 min-w-0">
                                        <FileVideo size={24} className="text-blue-500 flex-shrink-0" />
                                        <div className="min-w-0">
                                            <p className="text-sm font-medium text-gray-900 truncate">{data.media.name}</p>
                                            <p className="text-sm text-gray-500">{formatFileSize(data.media.size)}</p>
                                        </div>
                                    </div>
                                    <button type="button" onClick={removeFile} className="flex-shrink-0 p-1 text-red-500 rounded hover:text-red-700 focus:outline-none">
                                        <X size={16} />
                                    </button>
                                </div>
                            )}
